...
class UpdateDebtRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'installments_count' => ['sometimes', 'integer', 'min:1', 'max:360'],
            'payment_dates' => ['sometimes', 'array', 'min:1'],
            'payment_dates.*' => ['date'],
        ];
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Only validate if both are present in the request
            if ($paymentDates !== null && $installmentsCount !== null) {
                if (count($paymentDates) !== (int) $installmentsCount) {
